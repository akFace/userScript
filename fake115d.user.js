  ...
  CreateDownloadTask = function (o) {
    ...
    cb = function (r) {
      ...
      if (rs.length === n) {
        ...
        if (n > 1) {
          btn = $(
            "<button style='background: #50A2FF;color: #fff;padding:6px 10px;border-radius:10px'>复制所有链接</button>"
          );
          con.append("<li/>");
        ...
        for (j = 0, len = rs.length; j < len; j++) {
          f = rs[j];
          con.append(
            `<li style="padding: 5px 0"><a style="color: #50A2FF;" href='${f.url.url}'>${f.file_name}</a></li>`
          );
        }
