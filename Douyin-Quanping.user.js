...
// @version      1.1.6
// @author       akFace
// @description  抖音全屏优化，自动清屏，隐藏控制栏/侧边栏/搜索栏-抖音网页版优化
...
// @downloadURL https://update.greasyfork.org/scripts/508672/%E6%8A%96%E9%9F%B3%E5%85%A8%E5%B1%8F%E8%87%AA%E5%8A%A8%E6%B8%85%E5%B1%8F.user.js
// @updateURL https://update.greasyfork.org/scripts/508672/%E6%8A%96%E9%9F%B3%E5%85%A8%E5%B1%8F%E8%87%AA%E5%8A%A8%E6%B8%85%E5%B1%8F.meta.js
// ==/UserScript==
...
(function () {
  ...
  style_Add.innerHTML = `.isCssFullScreen .xg-video-container{bottom: 0 !important;}
    ...
    .isCssFullScreen .positionBox{opacity: 0 !important;}
    `;
  if (document.head) {
  ...
  var initClear = function () {
    var isCleared = false;
    var lastClearTime = 0;
    var minClearInterval = 2000; // 最小清屏间隔时间（毫秒）
    ...
    function clearScreen() {
      try {
        var controlsDom = document.querySelectorAll(
          '[data-e2e="feed-active-video"]'
        );
        var wei = "xg-switch"; // 未清屏状态
        var wei1 = "xg-switch xg-switch-checked"; // 清屏状态
        if (controlsDom.length) {
          var qingping = controlsDom[0].getElementsByClassName("xg-switch");
          if (qingping.length && qingping[0].className == wei) {
            qingping[0].click();
            console.log("清屏了");
            isCleared = true;
          } else if (qingping.length && qingping[0].className == wei1) {
            if (isCleared) {
              console.log("已清屏");
              isCleared = false;
            }
