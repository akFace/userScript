...
(() => {
  ...
  const state = {
    ...
    clearTime:
      parseInt(localStorage.getItem(CONFIG.clearTime)) || ConstClearTime,
  };
  ...
  GM_registerMenuCommand(
    ...
    () => {
      ...
      const value = parseInt(input) || state.clearTime;
  ...
  var initClear = function () {
    var isCleared = false;
    var lastClearTime = 0;
    var minClearInterval = state.clearTime; // 最小清屏间隔时间（毫秒）
